...
#include <pools/SimpleThreadPool.h>
...
TEST(SimpleThreadPoolTest, ReportsItsConfiguredWorkerCount)
{
    SimpleThreadPool<3U> pool;

    EXPECT_EQ(pool.threadCount(), 3U);
}

TEST(SimpleThreadPoolTest, ExecutesASubmittedTask)
{
    SimpleThreadPool<2U> pool;
...
TEST(SimpleThreadPoolTest, RejectsSubmissionWhenTheQueueIsFull)
{
    SimpleThreadPool<1U, 1U> pool;
...
TEST(SimpleThreadPoolTest, AcceptsWorkFromMultipleSubmittingThreads)
{
    ...
    SimpleThreadPool<4U, taskCount * 2U> pool;
...
TEST(SimpleThreadPoolTest, ShutdownCompletesQueuedTasksAndRejectsNewTasks)
{
    SimpleThreadPool<1U, 4U> pool;
    std::atomic<std::size_t> completedTasks{0U};
    std::promise<void> workerStarted;
    auto started = workerStarted.get_future();
    std::promise<void> releaseWorker;
...
TEST(SimpleThreadPoolTest, DestructionCompletesAlreadyAcceptedTasks)
{
    std::atomic<std::size_t> completedTasks{0U};
