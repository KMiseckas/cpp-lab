...
/**
 * SPSC Ring buffer with false sharing issue resolved for head/tail.
 * - False sharing - when 2 or more threads have to write to a cache line with 2 members on it.
 * This introduces unneccessery overheard due to cache line invalidating on each thread write,
 * for the next thread.
 * - Atomic operations
...
private:
    ...
    // alignas with minimum size required for cache line on current platform to fix false sharing.
